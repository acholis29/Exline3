...
function tpv_print_buffer(data, title, callback) {
    ...
    if (data.hasOwnProperty('namelist')) {
        ...
        namelist = namelist.replace(/<[^>]*>/g, ',').replace(/([0-9]|[\-+#.|()])+?/g, '');    // remove html tag
        ...
        for (var j = 0; j < namelist.length; j++) {
            ...
            if (name.length > 4) {
                number++;
                buffer += txtrow.replace(/\@:dt:/g, number + '. ' + name.split('|')[0].toUpperCase());
            }
        }
        namelist = buffer;
    }
    ...
    var $prn_header = $SETTINGS.print_header + (data.status_prn !== undefined ? ' ' + data.status_prn : '');
    ...
    var name_exc = (data.excursion_alias !== "" ? data.excursion_alias : data.excursion);
    $data += strnewline
    $data += txtrowright.replace(/\@:dt:/g, 'Code : ').replace(/\@:dtr:/g, data.voucher)
    $data += txtrowright.replace(/\@:dt:/g, 'Date : ').replace(/\@:dtr:/g, data.date_tr)
    if (typeof (data.barcode) !== 'undefined') {
        $data += txtrowright.replace(/\@:dt:/g, 'Barcode : ').replace(/\@:dtr:/g, data.barcode)
    }
    $data += strnewline
    $data += txtrowright.replace(/\@:dt:/g, 'PAX : ').replace(/\@:dtr:/g, pax)
    $data += txtrow.replace(/\@:dt:/g, 'Clients Name : ')
    $data += "\n{\n\"type\": \"TEXT\",\n\"text\":\"" + data.guestname.toUpperCase() + "\",\n\"align\":\"LEFT\",\n\"fontSize\": 20,\n\"isBold\": true,\n\"isUnderline\": false\n},"
    $data += namelist
    $data += strnewline
    $data += "\n{\n\"type\": \"TEXT\",\n\"text\":\"EXCURSION\",\n\"align\":\"LEFT\",\n\"fontSize\": 25,\n\"isBold\": true,\n\"isUnderline\": false\n},"
    $data += "\n{\n\"type\": \"TEXT\",\n\"text\":\"" + name_exc.toUpperCase().trim() + "\",\n\"align\":\"LEFT\",\n\"fontSize\": 20,\n\"isBold\": true,\n\"isUnderline\": false\n},"
    $data += txtrowright.replace(/\@:dt:/g, 'Tour Date : ').replace(/\@:dtr:/g, data.pickup.split(' ')[0])
    if (typeof (data.meeting_point) !== 'undefined') {
        if (data.meeting_point.trim().length != 0) {
            $data += txtrowright.replace(/\@:dt:/g, 'Pickup : ').replace(/\@:dtr:/g, data.meeting_point)
        }
    }
    $data += txtrowright.replace(/\@:dt:/g, 'Pickup Time : ').replace(/\@:dtr:/g, data.pickup.split(' ')[1])
    $data += strnewline
    $data += "\n{\n\"type\": \"TEXT\",\n\"text\":\"HOTEL INFO\",\n\"align\":\"LEFT\",\n\"fontSize\": 25,\n\"isBold\": true,\n\"isUnderline\": false\n},"
    $data += "\n{\n\"type\": \"TEXT\",\n\"text\":\"" + data.hotel.toUpperCase().split('/')[0].trim() + "\",\n\"align\":\"LEFT\",\n\"fontSize\": 20,\n\"isBold\": true,\n\"isUnderline\": false\n},"
    $data += txtrowright.replace(/\@:dt:/g, 'Room Number : ').replace(/\@:dtr:/g, data.hotel_room.toUpperCase())
    $data += strnewline
    $data += txtrow.replace(/\@:dt:/g, 'Note : ')
    $data += txtrow.replace(/\@:dt:/g, data.remark_supplier.toUpperCase())
    $data += strnewline
    $data += "\n{\n\"type\": \"TEXT\",\n\"text\":\"PAYMENT\",\n\"align\":\"LEFT\",\n\"fontSize\": 25,\n\"isBold\": true,\n\"isUnderline\": false\n},"
    $data += txtrowright.replace(/\@:dt:/g, 'Price : ' + data.currency).replace(/\@:dtr:/g, format_currency(data.salesrate, false))
    if (promosum > 0) {
        $data += txtrowright.replace(/\@:dt:/g, 'Discount : ' + data.currency).replace(/\@:dtr:/g, format_currency(promosum, false))
    }
    if (surcharges.length) {
        $data += txtrow.replace(/\@:dt:/g, 'Surcharge : ')
        $data += surcharges
    }
    $data += txtrowright.replace(/\@:dt:/g, 'Total : ' + payment.split(':')[0]).replace(/\@:dtr:/g, format_currency(data.totalsales, false))
    ...
    $data += strnewline
    $data += txtrowright.replace(/\@:dt:/g, 'Rep. : ').replace(/\@:dtr:/g, data.rep.toUpperCase())
    $data += txtrowright.replace(/\@:dt:/g, 'Agent : ').replace(/\@:dtr:/g, data.agent.toUpperCase())
    ...
    */
    $data += txtrowright.replace(/\@:dt:/g, 'Supplier : ').replace(/\@:dtr:/g, data.supplier.toUpperCase())
                ...
                if (Array.isArray(d)) {
                    ...
                    $.each(d, function (m) {
                        ...
                        if (dtEntradas.totalbonosentradas > 0) {
                            ...
                            $data += strnewline;
                            $data += "\n{\n\"type\": \"TEXT\",\n\"text\":\"" + dtEntradas.nombreproducto + "\",\n\"align\":\"CENTER\",\n\"fontSize\": 20,\n\"isBold\": true,\n\"isUnderline\": false\n},"
                            $data += "\n{\n\"type\": \"TEXT\",\n\"text\":\"" + dtEntradas.descripcionproducto + "\",\n\"align\":\"CENTER\",\n\"fontSize\": 15,\n\"isBold\": false,\n\"isUnderline\": false\n},"
                            $data += strnewline;
                            ...
                            for (const mmm in tt) {
                                ...
                                for (const mmmm in $mmm) {
                                    ...
                                    if (mmmm === 'Barcode') {
                                        ...
                                        $data += txtrowright.replace(/\@:dt:/g, mmmm.toUpperCase()).replace(/\@:dtr:/g, $mmm[mmmm]);
                                    }
                    ...
                    postITOS(module_prn, $data.substring(0, $data.length - 1), function (e) {
                        $($tpv_result).filter(function (i, n) {
                            var d = $.parseJSON(e)
                            if (n.val === d.resultCode) {
                                if (d.resultCode < 0) {
...
function tpv_printbarcode(e, callback) {
    var $barcode_type = $($tpvarr_barcode).filter(function (i, n) { return n.val_plugin === $SETTINGS.barcode_type; });
    var str = "\n{\n\"type\":\"BARCODE\",\n\"content\":\"" + e.substring(0, parseInt($SETTINGS.barcode_length)) + "\",\n\"height\": 60,\n\"margin\": 5,\n\"scale\": 2,\n\"format\":\"" + $barcode_type[0].val + "\",\n\"align\": \"CENTER\"\n},"
...
function tpv_printqr(e, callback) {
    var str = "\n{\n\"type\":\"QR\",\n\"content\":\"" + e + "\",\n\"size\": \"" + $SETTINGS.tpv_qrsize + "\",\n\"align\": \"" + $SETTINGS.tpv_qrposition + "\"\n},"
...
function tpv_printfooter() {
    ...
    if ($SETTINGS.string.footer_branch_company == 'true') {
        str += "\n{\n\"type\": \"TEXT\",\n\"text\":\"" + $SETTINGS.branch.name + "\",\n\"align\":\"LEFT\",\n\"fontSize\": " + $def_print.fontsize + ",\n\"isBold\": true,\n\"isUnderline\": false\n},"
        str += "\n{\n\"type\": \"TEXT\",\n\"text\":\"" + $SETTINGS.string.address_of_branch + "\",\n\"align\":\"LEFT\",\n\"fontSize\": " + $def_print.fontsize + ",\n\"isBold\": false,\n\"isUnderline\": false\n},"
        str += "\n{\n\"type\": \"TEXT\",\n\"text\":\"Phone : " + $SETTINGS.string.phone_branch + "\",\n\"align\":\"LEFT\",\n\"fontSize\": " + $def_print.fontsize + ",\n\"isBold\": false,\n\"isUnderline\": false\n},"
    }
...
function tpv_printbillccr(dtticket, msg, callback) {
    ...
    var amount = dtticket.Amount.substring(0, dtticket.Amount.length - 2) + '.' + dtticket.Amount.slice(dtticket.Amount.length - 2)
    var $companyname = "\n{\n\"type\": \"TEXT\",\n\"text\":\"" + $SETTINGS.company.name + "\",\n\"align\":\"CENTER\",\n\"fontSize\": 20,\n\"isBold\": true,\n\"isUnderline\": false\n},"
    var $tgl = dtticket.Date.substring(0, 4) + '-' + dtticket.Date.substring(4, 6) + '-' + dtticket.Date.substring(6, 8)
    var $jam = dtticket.Time.substring(0, 2) + ':' + dtticket.Time.substring(2, 4)
    var $data = "{\n\"typeFace\":\"Sans-Serif\",\n\"advancePaper\": true,\n\"letterSpacing\": 2,\n\"grayLevel\": 2,\n\n\"rows\":["
    $data += tpv_printlogo() + $companyname
    $data += strnewline
    $data += txtrowright.replace(/\@:dt:/g, 'ID. : ' + dtticket.Id).replace(/\@:dtr:/g, 'MID : ' + dtticket.MerchantId)
    $data += txtrow.replace(/\@:dt:/g, 'Card Type ' + dtticket.CardType)
    $data += "\n{\n\"type\": \"TEXT\",\n\"text\":\"" + dtticket.CardNumber + "\",\n\"align\":\"LEFT\",\n\"fontSize\": 22,\n\"isBold\": true,\n\"isUnderline\": false\n},"
    ...
    $data += txtrowright.replace(/\@:dt:/g, 'SALES ').replace(/\@:dtr:/g, 'Date : ' + date_format($tgl, 'dd MMM yyyy') + ' ' + $jam)
    $data += txtrowright.replace(/\@:dt:/g, 'APPR CODE :' + dtticket.Authorization).replace(/\@:dtr:/g, ' ')
    $data += "\n{\n\"type\": \"TEXT\",\n\"text\":\"TOTAL : " + dtticket.Currency + "  " + amount + "\",\n\"align\":\"RIGHT\",\n\"fontSize\": 26,\n\"isBold\": true,\n\"isUnderline\": false\n},"
    $data += strnewline
    $data += txtrowright.replace(/\@:dt:/g, 'AID. : ' + dtticket.AID).replace(/\@:dtr:/g, 'ARC. : ' + dtticket.ARC)
    $data += txtrowright.replace(/\@:dt:/g, 'ATC. : ' + dtticket.ATC).replace(/\@:dtr:/g, 'PSN. : ' + dtticket.PSN)
    $data += txtrowright.replace(/\@:dt:/g, 'TERM. : ' + dtticket.TerminalId).replace(/\@:dtr:/g, ' ')
    $data += strnewline
    ...
    $data += txtrowright.replace(/\@:dt:/g, '----' + msg + '----').replace(/\@:dtr:/g, ' ')
