 ...
 * @author Ben Plum
 * @version 0.1.4 + MODIFIED BY ipunk.vizard@gmail.com
 *
...
if (jQuery) (function($) {
		...
		destroy: function() {
			return $(this).each(function(i) {
			    ...
			    */
                // AFTER
                var $input = $(this);
                if($input.parent('div').hasClass('stepper')) {
	...
	function _stepDown(e) {
        ...
        if(settings.onStep !== null) {
            settings.onStep(data.$input.val());
        }
        /* IPUNK! */

		if (!data.$stepper.hasClass("disabled")) {
			var direction = $(e.target).hasClass("up"); //jika true, maka (+/up) dan jika false maka (-/down)
	...
	function _doStep(data, direction, firstStep) {
		...
		if (value != originalValue) {
            ...
            if(settings.onChange !== null) {
                settings.onChange(data.$input, originalValue, value);
            }
            /* IPUNK! */
		}
