...
function generate_parameters(act) {
    ...
    switch (act) {
        case 'before-checkout':
            ...
            buffer += "insert into TMP_MFExcursionHeader select "
            ...
            // contact
            buffer += "insert into TMP_MFExcursionContactDetail select "
            buffer += "''" + SESSION.transaction_mf + "'',"
            buffer += "''" + SESSION.transaction_number + "'',"
            ...
            $('#pay-input li.root').each(function (i, e) {
                ...
                if ($checkbox.is(':checked')) {
                    buffer += "insert into TMP_MFExcursionPaymentDetail select "
                    buffer += "''" + SESSION.transaction_mf + "'',"
                    buffer += "''" + SESSION.transaction_number + "'',"
            ...
            if ($SETTINGS.required_guest_details == 'true') {
                ...
                for (var i = 0; i < $data.length; i++) {
                    ...
                    buffer += "insert into TMP_MFExcursionNameList select "
                    buffer += "''" + SESSION.transaction_mf + "'',"
