...
$(document).ready(function () {
  ...
  document.addEventListener(
    ...
    function () {
      ...
      // DB_FILE = "db_exc" + $SETTINGS.company.id + ".db";
  ...
  $excursion = $app.autocomplete({
    ...
    source: function (autocomplete, query, render) {
      var input_location =
        $location.value.length != 0 ? $location.value[0].idx_state : "";
      var results = [];
    ...
    onChange: function (autocomplete, data) {
      $("#i-allotment-calendar").val("");
      $(div_search_excursion)
        .find("input[type=text]")
        .val(data[0].excursion.toUpperCase());
      $(div_search_excursion)
        .find("input[type=hidden]")
        .val(data[0].idx_excursion);
  ...
  $("#input-agent").on("click", function () {
    if ($(div_search_agent).hasClass("disabled")) {
      $app.alert(
        "You can't change agent while you already have make one booking!",
        $STRING.info_information
      );
    }
  });
  ...
  $("#i-allotment-calendar").on("change", function () {
    ...
    if ($SETTINGS.lp_status == "true") {
      ...
      if (
        $return_HIBDisponible.DatosResult !== null &&
        $return_HIBDisponible.DatosResult !== undefined
      ) {
  ...
  $("#view-price-detail").on("click", function () {
    ...
    if (pupp == "") {
      $app.alert(
        "PUPP for this product is unknown!",
        $STRING.info_incorrect_data
      );
      return false;
    }
    ...
    $("#detail-hotel").text(
      $hotel_area.value[0].hotel + ($room.val() ? " / " + $room.val() : "")
    );
  ...
  $("#add_to_cart").on("click", function () {
    ...
    if ($SETTINGS.lp_status == "true") {
      $paramReservaAforo.ConexionIacpos =
        $SETTINGS.loroparque.general.ConexionIacpos;
      $paramReservaAforo.clienteAPI = $SETTINGS.loroparque.clienteAPI;
      $paramReservaAforo.IdentificadorUnico =
        $SETTINGS.loroparque.clienteAPI.Id;
    ...
    var buttons1 = [
      {
        ...
        onClick: function () {
          ...
          $("#promotion-info").append(
            "Type a <b>code</b> or registered <b>guest name</b>!"
          );
  ...
  $("#checkout-finish").on("click", function () {
    ...
    if (!string_to_boolean($SETTINGS.payment_manual)) {
      get_cash_minus(
        SESSION.transaction_payment,
        get_pay_array(),
        function (val_to_convert) {
          ...
          if (val_to_convert != 0) {
            $app.alert(
              "Amount of payment isn't match with total payment!",
              $STRING.info_required_input,
              function () { }
            );
          } else {
            if (
              $("#checkout-paymethod").val() === "cc" ||
              $("#checkout-paymethod").val() === "cp"
            ) {
                ...
                $itos_payment.init(function (r) {
                  if (r.resultCode === 1000) {
                    ...
                    $itos_payment.payment(
                      SESSION.transaction_number_last,
                      amountcc,
                      function (rr) {
                        if (rr.resultCode === 0) {
                          //print bill for CC
          ...
          onClick: function () {
            checkout_reset(is_online, function () {
              if (is_online) {
                $MODE = "online";
                $app.alert(
                  "You are now connected to server by using <b>" +
  ...
  $("#checkout-paygate").on("click", function () {
    ...
    if ($email.val() == "") {
      $app.alert("Email is empty!", $STRING.info_required_input, function () {
        $email.focus();
      });
      return false;
    } else {
...
function payment_method_initialize(callback) {
  ...
  if (string_to_boolean($SETTINGS.payment_combinable)) {
    buffer +=
      '<option value="cp" ' +
      ($SETTINGS.payment_default == "cp" ? "selected" : "") +
      ">Combinable Payment</option>";
  }

  buffer +=
    '<option value="ca" ' +
    ($SETTINGS.payment_default == "ca" ? "selected" : "") +
    ">Pay with Cash</option>";
  buffer +=
    '<option value="cc" ' +
    ($SETTINGS.payment_default == "cc" ? "selected" : "") +
    ">Pay with Credit Card</option>";
  buffer +=
    '<option value="pot" ' +
    ($SETTINGS.payment_default == "pot" ? "selected" : "") +
    ">" +
    label_pot +
    "</option>";
...
function guest_form_generate(callback) {
  ...
  $("#cart-items li").each(function (i, e) {
    ...
    buffer +=
      '<div class="content-block-title"><b>' + $data.excursion + "</b></div>";
...
function get_pickuptime() {
  ...
  serialize +=
    "&ids=" +
    ($excursion.value[0].idx_sub !== null ? $excursion.value[0].idx_sub : "");
...
function list_maxpax(callback) {
  ...
  if ($MODE == "online") {
    ...
    if (exc.pf_adult != 0) {
      ...
      $("#div-booking-pax").append(
        "<i style='color:red;'>Incorrect setup for minimum adult pax!</i>"
      );
    }
    $loader.hide();
    callback();
...
function allotment_list(callback, reset, month, year) {
  ...
  $loader.show();
  $status.set(
    "ASKING ALLOTMENT DATA FROM SERVER<br><b>" +
    monthNames[month - 1] +
    " " +
    year +
    "</b>"
  ...
  if ($f_settings.lp_status.get() == "true") {
    var $dataselect = obj_props_to_lower(
      $(div_search_excursion).data("select")
    );
    if (
      $dataselect.conexios !== null ||
      $dataselect.enlace_menu !== null ||
      $dataselect.group_internet !== null ||
      $dataselect.product_id !== null
    ) {
      ...
      if (
        $dataselect.product_id !== undefined &&
