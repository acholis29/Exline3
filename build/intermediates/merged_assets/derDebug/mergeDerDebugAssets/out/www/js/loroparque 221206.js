...
var $lp = function () {
    return {
            ...
            $.ajax({
                ...
                success: function (r) {
                    ...
                    dbwebsql_lp.transaction(function (tx) {
                        ...
                        tbl_trxitemBono += " [EsBono], ";
                        tbl_trxitemBono += " [ProductoId], ";
                        tbl_trxitemBono += " [BonoId], ";
                        tbl_trxitemBono += " [NombreProducto], ";
                        tbl_trxitemBono += " [DescripcionProducto], ";
                        tbl_trxitemBono += " [Cantidad], ";
                        tbl_trxitemBono += " [GrupoInternet], ";
                        tbl_trxitemBono += " [TotalEntradas], ";
                        tbl_trxitemBono += " [NombreGrupoInternet], ";
                        tbl_trxitemBono += " [PrecioBruto], ";
                        tbl_trxitemBono += " [PvpProducto], ";
                        tbl_trxitemBono += " [FechaVisita], ";
                        tbl_trxitemBono += " [AforosConsumidos], ";
                ...
                error: function (e) {
                    // $app.alert(JSON.stringify(e), 'Error Login Loroparque');
                    //$loader.hide();
        ...
        HIBDisponible: function (param, month, year, callback) {
            console.log('HIBDisponible', param);
        ...
        ReservaAforo: function (param, callback) {
            console.log('ReservaAforo', param);
                ...
                success: function (r) {
                    if (r.Mensajes.length !== 0) {
                        ...
                        if (r.DatosResult.MensajeRespuesta == "OK") {
                            ...
                            var $arr_lp_filterprodID = $($return_HIBDisponible.DatosResult.AforosDisponibles[0].GruposInternetIdDisponible).filter(function (i, n) { return n.ProductoId === $SETTINGS.loroparque.product_id; });
                            console.log($arr_lp_filterprodID)
                            console.log($arr_lp_filterprodID[0].ProductoId)
                            ...
                            exec_Sql(dbwebsql_lp, ssql_dbwebsql, function (r) {
                                ...
                                if (len == 0) {
                                    ...
                                    exec_Sql(dbwebsql_lp, ssql_dbwebsql, function (rr) {
                                        ssql_dbwebsql = str_trx_item($arr_lp_filterprodID[0]);
                                        exec_Sql(dbwebsql_lp, ssql_dbwebsql, function (rrr) {
                                            if ($SETTINGS.loroparque.bono_id !== '') {
                                                var ssql_dbwebsqlbono = str_trx_itembono($arr_lp_filterprodID[0]);
                                                exec_Sql(dbwebsql_lp, ssql_dbwebsqlbono, function (rrr) {
                            ...
                            $app.alert(JSON.stringify(r.DatosResult.MensajeRespuesta), 'ERROR');
                            return false;
                ...
                if (r.rows.length > 0) {
                    ...
                    $.each(r.rows, function (k1, v1) {
                        ...
                        exec_Sql(dbwebsql_lp, ssql_dbwebsql, function (r_trxitem) {
                            ...
                            $.ajax({
                                url: get_core_jsonfile('', 'loroparque.json'),
                                type: 'GET',
                                dataType: 'json',
                                success: function (result) {
                                    ...
                                    $.each(r_trxitem.rows, function (k, v) {
                                        ...
                                        if (v.BonoId == '') {
                                            ...
                                            */
                                            ...
                                            inserctionLP($baffer_tmplpinsercion, function () {
                                                callback();
                                            })
                                            ...
                                            trx_itemBono($dtitem_bonoid.ProductosBono[0], v, $dtitem_bonoid.ProductosBono.length,
                                                function (re) {
                                                    console.log(re)
                                                    $baffer_tmplpinsercion.ProductosCompra.push(re);
                                                    $.each($dtitem_bonoid.ProductosBono, function (kBono, vBono) {
                                                        ...
                                                        $baffer_tmplpinsercion.ProductosCompra[0].PrecioBruto = v.PrecioBruto;
                                                        $baffer_tmplpinsercion.ProductosCompra[0].PvpProducto = v.PvpProducto;
                                    ...
                                    function inserctionLP($baffer_tmplpinsercion1, callback1) {

                                        $.ajax({
                                            url: $SETTINGS.loroparque.site.url + 'Insercion',
                                            data: JSON.stringify($baffer_tmplpinsercion1),
                                            type: "POST",
                                            processData: false,
                                            contentType: 'application/json',
                                            success: function (r) {
        ...
        InsercionNonBonoId: function (callback) {
        ...
        insert_ticket: function (data, idx_mf, callback) {
            ...
            serialize += '&idx_trx=&idx_mf=' + idx_mf + '&note=' + JSON.stringify(data) + '&' + p.substring(0, p.length - 1)
        ...
        delete_trx_websql: function (idx_mf, idx_trx) {
            dbwebsql_lp.transaction(function (tx) {
                tx.executeSql("DELETE FROM trx_header WHERE idx_mf='" + idx_mf + "'");
                tx.executeSql("DELETE FROM trx_item WHERE idx_mf='" + idx_mf + "' AND idx_trx='" + idx_trx + "'");
                tx.executeSql("DELETE FROM trx_itemBono WHERE idx_mf='" + idx_mf + "' AND idx_trx='" + idx_trx + "'");
            });
        },
        resync_ticket: function (data, callback) {
            var is_online = $MODE == 'online' ? true : false;

            $app.modal({
                title: $STRING.info_confirmation,
                text: 'Record ticket not found<br>are you sure resync this booking ?',
                buttons: [
                    {
                        text: 'Yes',
                        bold: true,
                        onClick: function () {
                            ...
                            SESSION.transaction_mf = data.idx_mfexcursion;

                            var today = new Date(data.pickup);
                            var month = today.getMonth() + 1;
                            var year = today.getFullYear() + '';

                            var module = get_core_module();
                            var serialize = '&act=form-search-excursion'
                            serialize += '&search=' + encodeURIComponent(data.excursion)
                            serialize += '&idr=' + $SETTINGS.user.id
                            serialize += '&ids=' + ''
                            serialize += '&idl=' + $SETTINGS.language.id
                            $status.set('INITIALIZE DATA');
                            $loader.show();
                            $.ajax({
                                url: module,
                                data: serialize,
                                ...
                                success: function (r) {
                                    console.log(r);
                                    $loader.hide();
                                    $status.set('INITIALIZE HIBDisponible');
                                    $loader.show();

                                    $lp.HIBDisponible(r[0], month, year, function (rHD) {
                                        console.log(rHD)
                                        if (rHD.DatosResult !== null) {

                                            var $prod_lp = $(rHD.DatosResult.AforosDisponibles[0].GruposInternetIdDisponible).filter(function (i, n) {
                                                return n.ProductoId === r[0].product_id;
                                            });

                                            $SETTINGS.loroparque.product_id = r[0].product_id;
                                            ...
                                            if ($prod_lp.length !== 0) {
                                                ...
                                                if ($prod_lp[0].BonoId == '') {
                                                    if ($prod_lp[0].ProductosRecintos.length !== 0) {
                                                        ...
                                                        var s = $($prod_lp[0].ProductosRecintos[0].ProductosRecintosSesiones).filter(function (i, n) {
                                                            return n.Fecha === data.pickup.split(" ")[0].replace(/\-/g, '');
                                                        });
                                                        $SETTINGS.loroparque.HoraInicio = s[0].HoraInicio;
                                                        console.log(s)
                                                    ...
                                                    if ($prod_lp[0].ProductosBono[0].ProductosRecintosComponenteDelBono[0].length !== 0) {
                                                        ...
                                                        var s = $prod_lp[0].ProductosBono[0].ProductosRecintosComponenteDelBono;
                                                        console.log(s)
                                                ...
                                                $SETTINGS.loroparque.general.ConexionIacpos = r[0].conexios;
                                                $SETTINGS.loroparque.RecintosSesionId = data.idx_transaction.split('#')[1];
                                                $SETTINGS.loroparque.Cantidad = data.pax;
                                                $SETTINGS.loroparque.Fecha = data.pickup.split(" ")[0];
                                                // 
                                                ...
                                                $status.set('INITIALIZE ReservaAforo');
                                                $loader.show();
                                                $lp.ReservaAforo($paramReservaAforo, function (rAforo) {
                                                    console.log('ReservaAforo', rAforo)
                                                    //--- batasi looping cuman 1 kali
                                                    console.log(rAforo);
                                                    $lp.Insercion(function (rInsercion) {
                                                        checkout_reset(is_online, function () {
                                                            $loader.hide();
                                                            $("#fab-printticket_lp").trigger("click");
                                                        });
                                                    })
                                                });
...
function str_trx_item(dtitem) {
    ...
    var ssql_dbwebsqlr = "INSERT INTO trx_item VALUES ('"
...
function str_trx_itembono(dtitem) {
    console.log(dtitem);
    var ss = {
        EsBono: dtitem.EsBono,
        ProductoId: dtitem.ProductoId,
        BonoId: dtitem.BonoId,
        NombreProducto: dtitem.NombreProducto.replace(/\n/g, '').replace(/\r/g, ''),
        DescripcionProducto: dtitem.DescripcionProducto,
        Cantidad: $SETTINGS.loroparque.Cantidad,
        GrupoInternet: $SETTINGS.loroparque.general.GruposInternet,
        TotalEntradas: $SETTINGS.loroparque.Cantidad,
        NombreGrupoInternet: $SETTINGS.loroparque.general.GruposInternet,
        PrecioBruto: parseFloat(dtitem.BaseImponible), //* v.Cantidad,
        PvpProducto: parseFloat(dtitem.PVPInternet),// * v.Cantidad,
        FechaVisita: date_format($SETTINGS.loroparque.Fecha, 'yyyy/MM/dd'),
        AforosConsumidos: []
    };
    var sss = "INSERT INTO trx_itemBono VALUES ('"
        + SESSION.transaction_mf + "','" // idx_mf
        + SESSION.transaction_id + "','" // idx_trx
        + dtitem.EsBono + "','"
        + dtitem.ProductoId + "','"
...
function trx_itemBono(dtitem, v, lenrec, callback) {
    console.log(lenrec)
    console.log(dtitem)
    console.log(dtitem.ProductosRecintosComponenteDelBono)

    var itemBonos = '';
    /*
        for (let i = 0; i < lenrec; i++) {
            if (dtitem.ProductosRecintosComponenteDelBono !== null) {
    
                $.each(dtitem.ProductosRecintosComponenteDelBono, function (kBono, vBono) {
                    itemBonos.AforosConsumidos.push(trx_itemBonoRecinto(dtitem.ProductosRecintosComponenteDelBono[kBono], v));
                });
            }
    
        }
    */
    ssql_dbwebsql = "SELECT DISTINCT * FROM trx_itemBono WHERE [idx_mf]='" + SESSION.transaction_mf + "'";
    exec_Sql(dbwebsql_lp, ssql_dbwebsql, function (r) {
        console.log(r)
        var len = r.rows.length, i;
        if (len !== 0) {
            ...
            $.each(r.rows, function (k, vv) {
                itemBonos = itembono(k, v);
            });
...
function itembono(dtitem, v) {
    var itemBono =
    {
        EsBono: v.EsBono,
        ProductoId: v.BonoId,
        BonoId: v.BonoId,
        NombreProducto: dtitem.NombreProducto,
        DescripcionProducto: dtitem.DescripcionProducto,
        Cantidad: v.Cantidad,
