$(document).ready(function () {
    ...
    $('#printer-print').on('click', function (e) {
        ...
        var device_print = $('#printer-list').find(':checked').parents('li').data('item');
        ...
        if (device.platform != 'iOS') {
            ...
            printer_device_connect(device_print, function () {
                ...
                if (typeof data[0] == 'string') {
                    if (title == 'TICKET') {
                        ...
                        printer_print_ticket(data[0].split("#")[0], data[0].split("#")[3], $SETTINGS.print_header);
                        ...
                        if (data[i].idx_mfexcursion) {
                            ...
                            if ($SETTINGS.lp_status == 'true') {
                                ...
                                if (data[i].status_prn == undefined) {
                                    printer_print_ticket(data[i].idx_mfexcursion, data[i].idx_transaction.split('#')[2], $SETTINGS.print_header)
                                }
                            }
                        }
                    }
            ...
            printer_device_connect_iOS(device_print,
...
function printer_print_buffer_array(data, title) {
    ...
    buffer = "{br}{center}{w}{b}" + $SETTINGS.print_header + "{/b}{/w}"
    buffer += "{br}{center}{s}PRINTED ON " + date_format(get_date_time(), 'dd/MM/yyyy HH:mm') + '{/s}';
    ...
    buffer += "{br}{s}" + $SETTINGS.string.address_of_branch.toUpperCase() + "{/s}";
...
function printer_print_buffer(data) {
    ...
    // setup
    pax = (parseInt(data.paxa) + parseInt(data.paxc) + parseInt(data.paxi)) + ' (A:' + data.paxa + ', C:' + data.paxc + ', I:' + data.paxi + ')';

    //buffer = line_feed + "{b}VOUCHER " + (data.status_prn == undefined ? '' : data.status_prn) + "{/b}" + line_feed
    buffer = line_feed + "{b}" + $SETTINGS.print_header + "{/b}" + line_feed
    ...
    buffer += line_feed + "@:separator:"
    buffer += line_feed + "Excursion   : " + (typeof (data.excursion_code) !== 'undefined' ? data.excursion_code : '')
    ...
    buffer += line_feed + "@:separator:"
    buffer += line_feed + "Excursion : " + data.currency + ' ' + format_currency(data.salesrate, false)
    if (promosum > 0) {
        buffer += line_feed + 'Discount  : ' + data.currency + ' -' + format_currency(promosum, false)
    }
...
function get_company_logo(initial) {
    var initial_x = identify_company($SETTINGS.company.id) == 'DEMO' ? 'sts' : initial.toLowerCase();
    var file = {
        //path:   $SETTINGS.server + '/images/logo/logom.png',
        path: 'img/logo_' + initial_x + '.jpg',
        height: 128,
        width: 300
    }
    return file;
                ...
                if (Array.isArray(d)) {
                    ...
                    $.each(d, function (m) {
                        ...
                        if (dtEntradas.totalbonosentradas !== '0') {
                            ...
                            printer_print_image(logo, '', function () {
                                input = line_feed + "{b}" + note + "{/b}" + line_feed + line_feed
                                input += "{b}" + dtEntradas.nombreproducto + "{/b}" + line_feed
                                input += "{s}" + dtEntradas.descripcionproducto + "{/s}" + line_feed
                                input += "{left}" + ($SETTINGS.print_size == 'small' ? '{s}' : '');
                                var _barcode = '';
                                ...
                                for (const mmm in tt) {
                                    ...
                                    for (const mmmm in $mmm) {
                                        if (mmmm === 'Barcode') {
                                            ...
                                            input += line_feed + mmmm.toUpperCase() + " : " + $mmm[mmmm]
                                        }
                                    }
                                    input += line_feed
                                ...
                                input += line_feed
                                input += ($SETTINGS.print_size == 'small' ? '{/s}' : '') + "{/left}"

                                var input2 = '';
                                printer_print_text(input, function () {
                                    printQRCodeJS(_barcode, function (r) {
                                        input2 = "{left}" + ($SETTINGS.print_size == 'small' ? '{s}' : '');
                                        input2 += _barcode
                                        input2 += line_feed
                                        input2 += line_feed
                                        input2 += line_feed + ".................cut here................."
                                        input2 += line_feed
                                        input2 += line_feed
                                        input2 += line_feed
                                        input2 += ($SETTINGS.print_size == 'small' ? '{/s}' : '') + "{/left}"
                            ...
                            printer_print_image(logo, '', function () {
                                input = line_feed + "{b}" + $SETTINGS.print_header + "{/b}" + line_feed
                                input += "{left}" + ($SETTINGS.print_size == 'small' ? '{s}' : '');
                                    ...
                                    if (typeof dtEntradas[mm] !== 'object') {
                                        if (mm === 'barcode') {
                                            ...
                                            if (dtEntradas[mm].length > 24) {
                                                ...
                                                input += line_feed + mm.toUpperCase() + " : " + dtEntradas[mm]
                                            }
                                        }
                                    }
                                };
...
function printMyBarcode(data, callback) {
    ...
    */
    ...
    var databarcode = data[0].barcode != undefined ? data[0].barcode : '49116500000000000001';
    ...
    if ($SETTINGS.print_barcode == 'true') {
        window.DatecsPrinter.setBarcode(setbarcode.dpalign,
            setbarcode.dpsmall,
            setbarcode.dpscale,
            setbarcode.dphri,
            setbarcode.dpheight,
